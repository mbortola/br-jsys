...
%-------------------------------------------------------------------------------diario modifiche
2.4 & 25/03/2008 & \AT & Correzione documento. \\ \hline
...
Il presente documento fornisce informazioni su come installare, disinstallare, avviare e utilizzare il sistema ``Br-jsys''.
...
\item messaggi di testo che informino l'utente sullo stato del prodotto;
\item il presente manuale che illustra le funzionalit\`a del linguaggio e accompagna l'utente nell'installazione, avvio e utilizzo del prodotto.
\end{itemize}

\section{Descrizione del sistema}
Il prodotto ``Br-jsys'' permetter\`a di testare la consistenza di \underline{\bo}. A tale scopo il software mette a disposizione, attraverso la propria interfaccia grafica, la possibilit\`a di inserimento di business rules, validazione, cancellazione e querying nel \underline{\rp}.

\section{Caratteristiche del servizio}
Una volta avviato, il sistema offre all'utente:
\begin{itemize}
...
\item[-] un validatore che consenta l'accettazione di nuove business rules, solamente se esse sono state scritte secondo le specifiche del linguaggio creato;
...
Questo manuale presenta il sistema ``Br-jsys'' illustrandone in modo completo le procedure di installazione, disinstallazione, avvio e utilizzo. 
...
\begin{itemize}
\item Nel capitolo 2 vengono illustrati i requisiti (software e hardware) necessari per una corretta installazione del prodotto;
...
\item nel capitolo 5 vengono analizzate e descritte le varie funzionalit\`a;
\item nel capitolo 6 viene spiegata la procedura di disintallazione del prodotto nei diversi sistemi operativi;
\item nel capitolo 7 viene presentato il linguaggio che definisce le \br;
...
\item[-] \textbf{Windows:} Windows2000 e WindowsXp home/professional.
\end{itemize}
\subsection{Java Runtime Environment (JRE)}
Essendo il prodotto realizzato con Java, per essere installato ed eseguito necessita di un ambiente Java Runtime Environment (\underline{JRE}) in versione 1.6.
\subsection{eXist}
Il prodotto utilizza il \underline{DBMS} eXist come sistema di gestione del \rp. \`E quindi indispensabile aver installato eXist in versione 1.2 prima di procedere all'installazione del ``Br-jsys''. L'installazione di eXist verr\`a presa in esame comunque nel capitolo successivo.
...
\end{center}
\item Comparir\`a la schermata di installazione: proseguire premendo il tasto ``Next'' ;
\begin{center}
\includegraphics[width=1\textwidth]{manuale_utente/installazione_Exist1.eps}\\
 figura 3.1.1: schermata iniziale
\end{center}
\item Verr\`a chiesto dove installare eXist: lasciare la directory di default (generalmente C:\textbackslash Programmi\textbackslash eXist per windows e  /home/\$USERNAME/eXist per sistemi linux). Premete quindi il tasto ``Next'' per proseguire;
\begin{center}
\includegraphics[width=1\textwidth]{manuale_utente/installazione_Exist3.eps}\\
 figura 3.1.2: scelta della directory
\end{center}
\item Scrivere  una password e reinserirla negli appositi campi dati per l'autenticazione al server DBMS. Premere ora sul tasto ``Next'' per proseguire. Attenzione la password di autenticazione inserita verr\`a richiesta all'avvio del prodotto ``Br-jsys'' assieme al nome utente ``\textbf{admin}''.
\begin{center}
...
\end{center}
\item Proseguite premendo il tasto ``Next'' fino a terminare l'installazione di eXist. Verr\`a visualizzata ora una schermata con alcune informazioni riguardanti il prodotto. Premere su ``Quit'' per completare l'installazione.
\begin{center}
...
L'installazione in sistemi Linux avviene attraverso uno script bash. Per prima cosa \`e necessario accedere, tramite un terminale (shell), alla cartella del cd di installazione e, successivamente avviare lo script di installazione tramite il comando \textbf{sh install.sh}. Quest'ultimo si occupa di posizionare correttamente i file e creare un link di avvio al prodotto.
Nell'illustrazione che segue si pu\`o vedere un corretto output dello script.\\
\begin{center}
...
L'installazione in sistemi Windows avviene attraverso un installer. Per prima cosa \`e necessario accedere al cd rom di installazione e successivamente fare doppio click sul file  \textbf{install.exe}. Quest'ultimo si occuper\`a di avviare un wizard di installazione.
\begin{center}
\includegraphics[width=1\textwidth]{manuale_utente/benvento.eps}\\
 figura 3.2.2: schermata iniziale in Windows
\end{center}
Occorre ora leggere e seguire le istruzioni fornite dal wizard di installazione e premere sul tasto ``Avanti'' per continuare.
...
Per l'installazione in sistemi operativi non descritti nelle precedenti sezioni, o in caso l'installazione (in forma di wizard o di script) non fosse andata a buon fine, \`e possibile installare il prodotto manualmente seguendo le seguenti istruzioni:
\begin{enumerate}
\item Individuate la cartella ``eXist'' nel vostro file-system;
...
\item copiate il contenuto della cartella ``data'' (\textbf{brjsys.jar} e \textbf{antlr-runtime-3.0.1.jar}) nella cartella ``brjsys'' che avete appena creato nel vostro file-system.
\end{enumerate}
Il prodotto \`e ora installato correttamente e per avviarlo utilizzate il file ``brjsys.jar''.


\chapter{Avvio del sistema}
\section{Avvio di eXist}
Affinch\`e il prodotto sia avviato e funzioni correttamente \`e necessario che sia attivo il server eXist. Completata l'installazione sar\`a sufficiente avviare eXist da uno degli \textit{shortcuts} creati dall'installazione. Baster\`a quindi un doppio click sull'icona ``eXist Database Startup'' creata.\\
Per lanciare invece eXist manualmente:
\begin{itemize}
\item[1-] Aprire la shell di Linux o il prompt dei comandi (Dos) di Windows;
\item[2-] accedere alla directory in cui \`e stato installato eXist e successivamente dentro la cartella `bin';
\item[3-] da shell Linux digitate:  \textbf{startup.sh}. Dal prompt dei comandi Windows digitate: \textbf{startup.bat}.
\end{itemize}	
...
\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/MainGui.eps}\\
 figura 4.2.1: schermata principale correttamente avviata
\end{center}
Se eXist non \`e stato avviato correttamente, o pi\`u semplicemente non \`e stato avviato, verr\`a mostrato all'utente un messaggio di errore indicante che il server eXist \`e spento (figura 4.2.2). \`E necessario quindi avviare il server eXist e ritentare il Login.
\begin{center}
...
Nel campo ``Regole Presenti'' l'utente pu\`o ricercare le business rules gi\`a presenti nel repository.
\subsection{Nome}
Rappresenta il nome della business rule che l'utente intende inserire. Il nome deve essere univoco all'interno del \rp; per questo motivo \`e importante prestare particolare attenzione al nome scelto. \`E tuttavia consigliabile definire, all'interno dell'azienda che utilizza il prodotto, delle regole di nomenclatura per i nomi delle business rules. Per facilitare un corretto inserimento, \`e presente una lista delle business rules inserite nel \rp. \`E cos\`i possibile visualizzare in modo rapido tutti i nomi delle business rules gi\`a inserite, evitandone l'inserimento di una con un nome gi\`a presente.

\subsection{Business Object Associato}
Rappresenta il \bo\ al quale si vuole associare la regola. Il linguaggio di definizione delle business rules offre la possibilit\`a di effettuare operazioni e controlli sui campi dati dei \bo\ da validare. Per questo, ad ogni business rule \`e necessario associare un \bo. Per facilitare l'inserimento di una nuova business rule (evitando cos\`i errori dovuti ad una digitazione errata del nome del \bo), il campo \bo\ associato \`e una \textit{combobox} che permette di scegliere uno dei \bo\ realmente presenti.

\subsection{Regola}
Rappresenta la regola scritta secondo la grammatica spiegata nel capitolo ``Descrizione del Linguaggio''. Per ovviare all'inserimento di \br\ con il campo regola identico, \`e possibile selezionare una delle business rule presenti nella lista e visualizzarne il campo ``regola'' nella \textit{statusbar} figura 5.2.3.
...
\subsection{Commento}
Rappresenta una descrizione sintetica del comportamento della regola. \`E opzionale e serve solo a fornire all'utente una descrizione pi\`u chiara e leggibile della regola stessa.

\subsection{Validazione e inserimento}
L'utente dovr\`a cliccare sul pulsante ``\textbf{Valida}''. Se l'inserimento viene effettuato con successo verr\`a visualizzato un messaggio di conferma, in caso contrario un messaggio d'errore indicher\`a all'utente cosa va modificato nella business rule affinch\`e venga validata correttamente. In alternativa si potr\`a cliccare sul tasto ``Cancella'' per rimuovere i dati immessi.
\begin{center}
...
\subsection{Esiti}
\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/br_inserita.eps}\\
 figura 5.2.6: esempio inserimento effettuato
\end{center} 
La figura 5.2.6 mostra l'esito di un corretto inserimento di una business rule. Essa \`e stata cos\`i validata e inserita nel \rp.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/errore_op_non_consentita.eps}\\
 figura 5.2.6.1: esempio di regola sintatticamente errata 
\end{center} 
La figura 5.2.6.1 mostra l'esito di un errato inserimento di una business rule. Essa non pu\`o essere validata e inserita nel \rp\ in quanto la regola immessa (in questo caso true=12) non \`e consentita.
\\
\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/errore_sintattico.eps}\\
 figura 5.2.6.2: esempio di regola errata
\end{center} 
La figura 5.2.6.2 mostra l'esito di un errato inserimento di una business rule. Essa non pu\`o essere validata e inserita nel \rp\ in quanto la regola immessa \`e stata scritta in maniera errata.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/err10diventa08.eps}\\
 figura 5.2.6.3: esempio di regola gi\`a inserita
\end{center} 
La figura 5.2.6.3 mostra l'esito di un errato inserimento di una business rule. Essa non pu\`o essere validata e inserita nel \rp\ in quanto vi \`e gi\`a un'occorrenza nel \rp. In questo caso viene selezionata la business rule trovata nella lista ``Regole Presenti'' e visualizzato il corrispondente campo ``regola'' nella \textit{statusbar}.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/errore_testo_gia_presente.eps}\\
 figura 5.2.6.4: esempio testo regola gi\`a presente
\end{center} 
La figura 5.2.6.4 mostra l'esito di un errato inserimento di una business rule. Essa non pu\`o essere validata e inserita nel \rp\ in quanto vi \`e gi\`a una business rule con quel campo . In questo caso viene selezionata tale business rule nella lista ``Regole Presenti'' e visualizzato il corrispondente campo ``regola'' nella \textit{statusbar}.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/errore_nome_regola_inaccettabile.eps}\\
 figura 5.2.6.5: esempio inserimento nome vuoto
\end{center} 
La figura 5.2.6.5 mostra l'esito di un errato inserimento di una business rule. Essa non pu\`o essere validata e inserita nel \rp\ in quanto il nome inserito \`e vuoto.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/errore_campodati.eps}\\
 figura 5.2.6.6: esempio inserimento campo dati non presente nel business object associato
\end{center} 
La figura 5.2.6.6 mostra l'esito di un errato inserimento di una business rule. Essa non pu\`o essere validata e inserita nel \rp\ in quanto il campo dati non \`e presente nel business object associato.
...
\begin{enumerate}
\item inserire nell'appostito campo (TextBox) il nome della business rule che si intende rimuovere e cliccare sul pulsante ``\textbf{Cerca}''. Se l'utente opera una o pi\`u ricerche, le \br\ trovate verranno selezionate nel campo ``Business Rules Selezionate'', mentre in ``Lista Completa'' sar\`a evidenziata solo l'ultima di queste. Si possono eventualmente deselezionare le business rules attraverso il tasto ``Deseleziona''. Viene infine introdotto l'uso di espressioni regolari per raffinare la ricerca; in questo caso invitiamo l'utente a consultare la documentazione fornita all' url:\\ \href{http://java.sun.com/j2se/1.4.2/docs/api/java/util/regex/Pattern.html}{http://java.sun.com/.../Pattern.html}.
...
\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/conferma_rimozione.eps}\\
...
\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/err11diventa09.eps}\\
 figura 5.3.1 messaggio di regola non trovata.
\end{center} 
La figura 5.3.1 mostra l'esito di un'errata rimozione di una business rule. Essa non pu\`o essere rimossa dal \rp\ in quanto non \`e stata trovata al suo interno.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/12diventa10.eps}\\
 figura 5.3.2 messaggio di repository vuoto.
\end{center} 
La figura 5.3.2 mostra l'esito di un'errata rimozione di una business rule. Essa non pu\`o essere rimossa dal \rp\ in quanto quest'ultimo non ne contiene alcuna.
...
\end{center}
La figura 5.4.1 mostra l'esito positivo di una query. In particolare, verr\`a visualizzato nel campo dati ``Risultati'' l'esito della query e nella barra sottostante il tempo di risposta (in millisecondi) impiegato da eXist.

\begin{center}
\includegraphics[width=0.5\textwidth]{manuale_utente/esempio_errore_sandbox_query_errata.eps}\\
...
\section{Disinstallazione su tutti i sitemi}
...
\begin{enumerate}
\item Individuate la cartella ``brjsys'' nel vostro file-system;
...
In questo capitolo viene descritto il linguaggio di formulazione di business rules. Una business rule serve a definire vincoli e regole da rispettare per un determinato \bo. Per questo motivo il linguaggio di definizione delle business rules \`e incentrato soprattutto sul confronto tra campi dati dei \bo\ ed espressioni algebriche o valori costanti. In una business rule si possono quindi ottenere valori attraverso l'utilizzo di espressioni algebriche, definire confronti tra questi valori e campi dati dei \bo\ e collegare infine questi confronti con operatori logici. L'insieme delle funzionalit\`a del linguaggio garantisce una buona espressivit\`a.
\`E molto importante per\`o chiarire l'utilizzo delle business rules; esse sono concepite per verificare che un oggetto rispetti determinati vincoli. I vincoli che si definiscono sono comunque vincoli oggettivi, ad esempio ``\texttt{se un valore è maggiore di 10 non deve essere minore di 10}'' e non vincoli soggettivi come ``\texttt{le entrate devono essere maggiori delle uscite}''. Mentre per la prima regola non \`e possibile che avvenga il contrario, per la seconda ci\`o \`e possibile per quanto negativo possa essere questo per un'azienda.

\section{Una business rule semplice}
Una business rule semplice \`e costituita da un singolo confronto. Ad esempio: ``prezzo\textgreater0'' . La regola appena scritta serve a verificare che ogni oggetto abbia un prezzo maggiore di zero. Nella regola ``prezzo'' \`e il nome di un campo dati dell'oggetto. Una regola minimale \`e quindi definita dal comfronto tra un campo dati dell'oggetto e un valore. \\
...
Le business rules (e di conseguenza i business objects) possono contenere campi dati e valori di tre tipi:
\begin{itemize}
...
Una business rule, in genere, non si limita a confrontare un campo dati con un valore, ma il pi\`u delle volte confronta un campo dati con un ``valore derivato''. Per valore derivato si intende un valore ottenuto algebricamente da altri valori. Ad esempio ``totale=prezzoBase*pezziVenduti''. Le operazioni algebriche consentite sono quelle base: + (somma), - (sottrazione), / (divisione), * (moltiplicazione).

\section{Funzioni algebriche}
Per rendere pi\`u semplice, chiaro e espressivo il linguaggio sono state introdotte alcune funzioni elementari, come la somma di un gruppo di valori (SUM), la media aritmetica (AVG) e il conteggio di un gruppo di valori(COUNT). Ad esempio \`e possibile scrivere regole come ``totale=SUM(listaPrezzi)'' dove \textit{SUM()} \`e la funzione e \textit{listaPrezzi} \`e un array di prezzi. Si pu\`o poi scrivere ``prezzoMedio=AVG(listaPrezzi)'' per la media aritmetica o ancora ``totale=prezzoBase* COUNT(listaPrezzi)'' per il conteggio degli elementi.\\
Le espressioni algebriche ammettono parentesizzazioni. \`E quindi possibile scrivere ``prezzo=costo*(ore/(produzione+3))'' .
\`E possibile introdurre nuove funzioni nel linguaggio a patto che queste siano supportate e riconosciute come tali dall'interprete del linguaggio. Nulla vieta infatti di introdurre funzioni come \textit{RADQ()} e introdurre quindi una funzione per la radice quadrata nel linguaggio.
...
\`E possibile inoltre effettuare operazioni tra matrici a patto che siano matrici omogenee.

\section{Confronti tra booleani}
Una business rule pu\`o contenere confronti tra booleani. Ad esempio ``difettoso=false''. Il campo dati deve essere un booleano e pu\`o essere confrontato solo con altri campi dati booleani o con valori come ``true'' e ``false''. 
\subsection{La funzione NOT()}
La funzione NOT() \`e una funzione propria dei booleani e serve a permettere la negazione di un valore booleano o di un espressione booleana. \`E possibile quindi scrivere ``NOT(difettoso)=true'', mentre non \`e possibile scrivere ``NOT(totale=prezzo*numero)''. ATTENZIONE: La funzione NOT() si applica solo a un valore o espressione booleana e non a un confronto!!!
...
Nel linguaggio \`e possibile inserire stringhe, ma le operazioni tra stringhe sono limitate ai soli confronti semplici. \`E quindi possibile scrivere ``nome=``prodotto'''' oppure ``nomeAzienda!=``IBM''''. I confronti ammessi sono = (uguale) e != (diverso).

\section{Connettori logici}
Il linguaggio fin qui descritto non garantisce una buona espressivit\`a, in quanto risulta difficile esprimere concetti complessi con un singolo confronto. Per fare questo \`e possibile connettere pi\`u confronti con dei connettori logici \textit{AND} e \textit{OR}. I connettori logici permettono di collegare pi\`u confronti fra loro.\\
...
\section{Il ``message''}
Il message \`e un modo per aiutare il debugging delle business rules, in quanto fornisce informazioni aggiuntive sullo stato di validazione di quest'ultime. Ad ogni confronto \`e quindi possibile associare un messaggio in formato stringa con la keyword \textit{message(``testo del messaggio'')}. Il message serve al momento dell'interpretazione della business rule, ovvero nel momento in cui si cerca di validare un \bo\ e questo potrebbe non rispettare uno dei confronti di una regola. Nel caso ci\`o avvenga, verr\`a mostrato all'utente il message riguardante il primo confronto fallito. Un message pu\`o quindi essere aggiunto al termine di ogni singolo confronto, includendo un messaggio chiaro che identifichi in modo chiaro per l'utente dove il \bo\ non rispetta una parte della business rule. Ad esempio \`e possibile scrivere: ``nomeProdotto=``Pc1'' message(``nome errato'') AND prezzoProdotto\textless1800 message(``prezzo non competitivo'')''. In questo modo se il \bo\ a cui si applica la regola dovesse avere un nome diverso da ``Pc1'' o un prezzo maggiore di 1800, verrebbe segnalato all'utente con un messaggio contenente il testo del message. I caratteri ammessi in un message sono: lettere dell'alfabeto maiuscole minuscole, numeri, spazi, simboli di punteggiatura come : `.' (punto),  `?' (punto di domanda),  `!' (punto esclamativo),  `\_' (underscore),  `-' (trattino, simbolo di sottrazione),  `\~' (tilde), `\^' (apice).
...
ERR06 & Nome regola inaccettabile & La business rule non pu\`o avere campo name vuoto. \\ \hline
