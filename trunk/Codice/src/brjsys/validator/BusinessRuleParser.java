...
public class BusinessRuleParser extends Parser {
	...
	public final start_return start() throws RecognitionException {
		...
		try {
			...
			//System.out.println(">>>>"+input.get(input.index()));
			throw error;}    finally {
			}
			return retval;
	}
			...
			// /../../media/sda1/ids/src/compiler/BusinessRule.g:92:2: e= exp ( conf e2= exp ) ( message )?
			{
				...
				// /../../media/sda1/ids/src/compiler/BusinessRule.g:93:3: conf e2= exp
				{
					...
					if(!input.toString(conf4.start,conf4.stop).equals("=") && !input.toString(conf4.start,conf4.stop).equals("!=")){//devono essere Float
						retval.value =compare(input.get(input.index()-1),e.value,compare(input.get(input.index()-1),e2.value,new Float(0).getClass(),false),false);
					}else {retval.value =compare(input.get(input.index()-1),e.value,e2.value,false);}
				...
				loop3:
					do {
							...
							// /../../media/sda1/ids/src/compiler/BusinessRule.g:105:3: OpA e= opA
						{
							...
							adaptor.addChild(root_0, e.getTree());
							retval.value =compare(input.get(input.index()-1),e.value,compare(input.get(input.index()-1),retval.value,new Float(0).getClass(),false),false);
				...
				loop5:
					do {
							...
							// /../../media/sda1/ids/src/compiler/BusinessRule.g:117:3: OpBool e= atom
						{
							...
							adaptor.addChild(root_0, e.getTree());
							retval.value =compare(input.get(input.index()-1),e.value,compare(input.get(input.index()-1),retval.value,new Boolean(true).getClass(),false),false);
