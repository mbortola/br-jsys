...
Il prodotto BR-jsys permette di testare la consistenza di business objects, altrimenti difficilmente verificabile tramite vincoli relazionali. A tale scopo il software mette a disposozione, attraverso la propria interfaccia grafica, la possibilità di inserire business rules validate nel repository, di cancellarne o di eseguire querying di test.
% da fare
...
% da fare
\subsection{Primo avvio e configurazione}
Una volta installate tutte le componenti necessarie, si potr\`a procedere all'avvio dell'applicazione. Apparir\`a la schermata iniziale (Gui), nella quale l' utente che accede per la prima volta al servizio dovr\`a digitare i dati di login:\\
username:\\
password:\\

    [Inserire immagine della schermata iniziale]\\
% da fare
...
Oggetto associato \textless nome associato\textgreater\ inesistente: validazione interrotta! & Inserimento di una business rule con un business object associato non valido. La validazione viene interrotta. \\ \hline
...
Operazione non consentita! \textless tipo 1\textgreater\ != \textless tipo 2\textgreater\. \textless tutta la business rule scritta fino all'errore\textgreater\: validazione interrotta! & Errore tra tipi. Esecuzione di un'operazione di confronto tra due tipi diversi. La validazione viene interrotta. \\ \hline
...
Regola \textless nome regola\textgreater\  gi\`a presente: inserimento fallito! & La regola ha un nome gi\`a presente nel repository. Non verr\`a quindi inserita.
\end{tabular} \\
\end{table}
