...
\begin{center}
\includegraphics[width=0.9\textwidth]{manuale_utente/schermata_esempio_inserimento.eps}\\
...
Nella figura 3.3 vediamo illustrata la schermata di cancellazione di una business rule. Per procedere all'eliminazione l'utente avr\`a due possibilit\`a:
\begin{itemize}
...
\end{itemize}
Se la business rule risulta presente nel repository, comparir\`a  nel campo ``Details'' i suoi dettagli (Nome, BO Associato, Regola, Commento). A questo punto baster\`a cliccare sul pulsante ``Rimuovi Selezionate''.  Se la cancellazione viene effettuata con successo verr\`a visualizzato un messaggio che ne conferma l'esito positivo, in caso contrario uno negativo.
\subsection{Sandbox}
...
Nella figura 3.4 vediamo illustrata la schermata di sandbox. Per procedere con l'interrogazione del database, l'utente dovr\`a inserire nell'appostito campo ``Query'', la query, scritta in linguaggio XQuery e cliccare sul pulsante ``Esegui''. Verr\`a visualizzato quindi nel campo dati ``Risultati'' l'esito della query.
