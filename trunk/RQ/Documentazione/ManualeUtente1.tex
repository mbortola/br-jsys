...
In questa sezione faremo una descrizione della grammatica da noi generata per rappresentare le business rules. La \textbr{struttura di una business rule} \`e molto semplice. Pu\`o essere formata da:
\begin{itemize}
\item una regola atomica;
\item un insieme di regole collegate tra loro attraverso gli operatori booleani (AND | OR).
\end{itemize}
Ogni singola \textbr{regola} che costituisce la business rule \`e costituita da un insieme di espressioni collegate tra loro attaverso un operatore di confronto (= | != | < | > | <= | >=). A ciascuna regola pu\`o venire associato un ``message'' (opzionale), col compito di fornire informazioni aggiuntive sullo stato di validazione della business rule. Una \textbr{espressione} \`e costituita a sua volta da campi dati che posso essere di 3 tipi diversi:
\begin{itemize}
\item float;
\item string;
\item bool. 
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.4\textwidth]{manuale_utente/schermata_inserimento.eps}
\end{center}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=0.4\textwidth]{manuale_utente/schermata_rimozione.eps}
\end{center}
...
\subsection{Sandbox}
\begin{figure}[htbp]ep
\begin{center}
\includegraphics[width=0.4\textwidth]{manuale_utente/schermata_sandbox.eps}
\end{center}
