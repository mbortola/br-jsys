...
In questa sezione faremo una descrizione della grammatica da noi generata per rappresentare le business rules. 
\subsection{Struttura business rule}
La business rule pu\`o essere formata da:
\begin{itemize}
...
Ogni singola regola \`e formata da espressioni collegate tra loro attaverso operatori di confronto (= | != | < | > | <= | >=). Alla regola pu\`o venire associato un ``message'' (opzionale), col compito di fornire informazioni aggiuntive sullo stato di validazione della business rule. 
...
\begin{itemize}
\item[-] un' elevata configurabilit\'a ed estendibilit\'a dei business object, mantenendo il rispetto dei controlli cablati.
