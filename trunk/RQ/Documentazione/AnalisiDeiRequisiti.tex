...
\newcommand{\lv}{3.3} % latest version
...
Il presente documento contiene una descrizione del prodotto oggetto del capitolato d'appalto designato dalla sigla \underline{BR-jsys}. Tale descrizione
comprende le sue funzionalit\`a, la tipologia di utenti ed il contesto d'uso. Prosegue con la descrizione e l'analisi dettagliata dei requisiti del prodotto in questione con l'ausilio di un formalismo grafico: i diagrammi use-case \underline{UML}. Vengono infine elencati tutti i requisiti individuati dalla
...
Il presente documento viene redatto al fine di identificare i requisiti presenti nel capitolato d'appalto denominato \underline{BR-jsys}, conseguire il raggiungimento dell'obiettivo richiesto e soddisfare le esigenze di tutti gli \underline{stakeholder} coinvolti nella realizzazione del progetto di cui sopra.
\section{Scopo del prodotto}
Il prodotto andr\`a inserito in un progetto pi\`u ampio. Il suo obiettivo \`e quello di testare la consistenza di \underline{business object}, altrimenti difficilmente verificabile tramite vincoli relazionali.
Verr\`a creato quindi un linguaggio per la definizione di \underline{business rule}. Quest'ultime verranno immagazzinate in un \textit{file \underline{repository}}; nel momento in cui l'utente avr\`a bisogno di verificare la consistenza di un \underline{business object}, un interprete si preoccuper\`a di invocare ed eseguire le regole che lo riguardano, stabilendo cos\`i se esso le rispetta tutte o meno.
Il progetto dovr\`a integrarsi infine con le componenti gi\`a sviluppate dall'azienda proponente. In particolare dovr\`a interfacciarsi con il loro interprete, il quale dovr\`a poi applicare le regole ai \underline{business object}.
...
Il sistema va a collocarsi all'interno di un progetto globale di automazione nella generazione del codice per la produzione di software gestionale. In particolare si occuper\`a della gestione delle \underline{business rule}, definendo un linguaggio con cui descriverle, oltre ad un sistema di archiviazione-recupero delle stesse.
\section{Funzioni del prodotto}
Il sistema si occupa di definire un linguaggio con cui descrivere \underline{business rule}. Pi\`u concretamente dovr\`a verificare che le \underline{business rule} inserite dall'utente rispettino il linguaggio sopra citato. In caso di esito positivo dovranno essere inserite in un \underline{repository}, dal quale potranno poi essere recuperate nel momento in cui si desideri verificare la consistenza di un \underline{business object}.
...
\begin{itemize}
\item{Le \underline{business rule} su cui si opera saranno di tipo \textit{\underline{Rejector}}. Una \underline{business rule} deve essere composta da uno o pi\`u confronti tra espressioni dello stesso tipo (eventualmente complesse).}
\item{I \underline{business object} che manipoleremo sono generalmente costituiti da campi dati semplici (numeri reali, booleani, stringhe), da un insieme di array o vettori omogenei tutti della stessa lunghezza (derivanti da tabelle) e da riferimenti ad altri \underline{business object} (ma non da array di \underline{business object}).
}
...
\subsection{Richiedi al \underline{DBMS}  di inserire nel \underline{repository}.}
...
\subsection{Recupera \underline{business rule} pertinenti.}
...
\textbf{Postcondizioni} Il \underline{DBMS} riceve la richiesta di recuperare delle \underline{business rule} dal \underline{repository}.
\subsection{Manda le \underline{business rule} all'interprete.}
\textbf{Attori coinvolti:} L'interprete esterno fornito dall'azienda che riceve le \underline{business rule} per poter validare il suo \underline{business object}.\\
...
\textbf{Precondizioni:} Il \underline{DBMS} ha terminato il recupero di tutte le \underline{business rule} che riguardano il \underline{business object} dato.\\
\textbf{Postcondizioni} L'interprete utilizzer\`a le \underline{business rule} appena ricevute.

\section{Requisiti funzionali}
\begin{itemize}
\item[F1]{Si dovr\`a creare un linguaggio per la rappresentazione di \underline{business rule}. [C04]}
\item[F2]{Il sistema dovr\`a fornire un validatore che consenta l'accettazione di nuove \underline{business rule} soltanto se esse sono scritte secondo le specifiche del linguaggio creato. [C04]}
\item[F3]{Il sistema dovr\`a inserire nel \underline{repository} la nuova \underline{business rule} dopo la sua accettazione da parte del validatore. [C04]}
...
\item[F7]{Il sistema dovr\`a consentire la cancellazione di \underline{business rule} all'interno del \underline{repository}. [I02]}
\item[F8]{Il sistema dovr\`a essere dotato di un'interfaccia grafica minimale che consenta di definire una nuova \underline{business rule} e una volta validata inserirla nel \underline{repository}. Tale interfaccia dovr\`a inoltre consentire di cancellare \underline{business rule} dal \underline{repository}. Tutto ci\`o servir\`a a testare in modo rapido e chiaro il corretto funzionamento del prodotto. [I02]}
...
\begin{itemize}
\item[NPo1]{Il formato di salvataggio delle \underline{business rule} e del \underline{repository} deve essere XML. [C04]}
\end{itemize}
...
\item[NPr2]{Ad ogni \underline{business rule} deve venire associata una stringa identificativa univoca per consentire un'efficiente indicizzazione delle \underline{business rule} nel \underline{repository}. [I02]}
\end{itemize}
...
\item[NQ2]{Il progetto verr\`a accompagnato da un manuale che descriver\`a il linguaggio di definizione delle \underline{business rule}. [C04]}
...
\textbf{Richiedi al \underline{DBMS} di cancellare dal \underline{repository}} & F7 - F8 \\ \hline
\end{tabular} \\ \\
...
C04 & F1, F2, F3, F4, F9, NPo1, NQ2, NQ3  \\ \hline
I01 & NU1, NU2, NQ1 \\ \hline
I02 & F5, F6, F7, F8, F10, F11, NU3, NPr1 \\ \hline
\end{tabular} \\
\end{center}
