...
Per quanto riguarda le rappresentazioni grafiche dell'architettura del sistema ``\underline{BR-jsys}'', ci siamo basati sulle regole definite da \underline{UML} 2.1. In particolare, ci siamo appoggiati all'uso di ``Poseidon for \underline{UML} - professional edition 6.0.2''. Per i diagrammi use-case ci siamo affidati invece a Dia 0.96.1.
\section{Standard di documentazione del codice}
Per le regole di documentazione del codice si faccia riferimento al documento ``\NdP'' allegato.
\section{Standard di denominazione di entit\`a e relazioni}
Per le regole di denominazione delle entit\`a, delle relazioni e degli oggetti, si faccia riferimento al documento ``\NdP'' allegato.
\section{Standard di programmazione}
Anche in questo caso, si faccia riferimento al documento ``\NdP'' allegato.
...
Dovr\`a operare nel seguente modo:
\paragraph{Attivit\`a di login:}
Verr\`a chiesto all'avvio di inserire l'username e la password per accedere ad \underline{eXist}, controllando eventuali errori.
\paragraph{Attivit\`a di inserimento:}
Si dovr\`a visualizzare una finestra che permetta la definizione di una nuova \br, mandarla al validatore ed inserirla nel \rp\ in caso di validazione corretta. Alternativamente si dovr\'a informare l'utente su cosa ha bloccato la procedura.
\paragraph{Attivit\`a di cancellazione:}
Verranno mostrate in un'apposita sezione tutte le \br, l'utente potr\`a selezionarne un numero arbitrario e procedere all` eliminazione delle stesse. Inoltre la finestra disporr\`a di un`apposita sezione dove potr\`a ricevere informazioni su una qualche \br\ nel \rp.
\paragraph{Attivit\`a di querying:}
Verr\`a visualizzata una finestra che permetter\`a di fare query al \rp\ in linguaggio \underline{XQuery}. Verr\`a visualizzato il risultato, il tempo di esecuzione ed in alternativa l'eventuale errore di scrittura. 
\begin{center}
...
-mainGui: JFrame & Frame principale che permette di accedere a tutte le funzionalit\`a. \\ \hline
-remGui: JFrame & Frame che mette a disposizione le operazioni di cancellazione. \\ \hline
-sandbox: JFrame & Frame che mette a disposizione le operazioni di querying. \\ \hline
-insertGui: JFrame & Frame che mette a disposizione le operazioni di inserimento.\\ \hline
\end{tabular}
\end{center}
\begin{center}
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
...
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
Metodo & Descrizione \\  \hline
+Validator(username: String password: String) \textbraceleft throws Exception \textbraceright& Costruttore di Validator. Richiede in ingresso le credenziali dell'utente per accedere al \underline{DBMS}. Qualora siano sbagliate viene lanciata un'eccezione.\\ \hline
+validate(rule:BusinessRule ):boolean \textbraceleft throws Exception \textbraceright& Unico metodo della componente che ricevuta in ingresso una \br\ ne effettua la validazione. Qualora si verifichino errori in fase di compilazione viene restituita un'eccezione gestita dal chiamante. Se la \br\ viene inserita, il metodo ritorna \textit{true}. Ritorna invece \textit{false} se la regola ha un nome che \`e gi\`a presente nel \rp.\\ \hline
\end{tabular}
\end{center}
...
\subsubsection{Attivit\`a svolte e dati trattati}
La componente, generata tramite un generatore di parser, dispone di numerosi campi dato per identificare i \underline{token}s della grammatica, nonch\`e vari metodi per la loro gestione. In definitiva si dovr\`a usare solo il suo costruttore.
\begin{center}
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
Metodo & Descrizione \\  \hline
...
\subsubsection{Attivit\`a svolte e dati trattati}
Questa componente, generata tramite un generatore automatico di parser, fornisce i campi dato per il riconoscimento dei \underline{token} in appoggio a quelli gi\`a forniti dalla componente \brp. Contiene vari metodi privati per il controllo sintattico e semantico, per il parsing e contiene infine classi innestate incaricate di generare l'\underline{AST}.
\begin{center}
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
Attributo & Descrizione \\  \hline
\underline{+\underline{token}Names: String[]} & Array che associa ogni \underline{token} ad un intero, Indispensabile per il parsing in XML dato che l'\underline{AST} rappresenta i nodi \underline{token} tramite gli interi che li rappresentano. \\ \hline
+endParsing: BusinessRuleParser.start\_return & Campo dati che contiene l'\underline{AST} da convertire in XML.\\ \hline 
\end{tabular}
\end{center}
\begin{center}
 \begin{tabular}{||p{6cm}||p{6cm}||}\hline
Metodo & Descrizione \\  \hline
+BusinessRuleParser(input: CommonTokenStream, associated: String) \textbraceleft throws\phantom{c}RecognitionException \textbraceright & Costruttore della componente. Il primo parametro identifica lo stream generato nelle fasi precedenti, il secondo dice qual'\`e il \bo\ associato alla regola. Se associated non corrisponde a nessun \underline{business object} viene sollevata un'eccezione.\\ \hline

+start(): BusinessRuleParser.start\_return \textbraceleft throws\phantom{c}RecognitionException \textbraceright & Risultato del parsing corretto di una \br. Contiene al suo interno l'\underline{AST}. Nel caso di errore nella validazione il chiamante dovr\`a gestire opportunamente l'eccezione.\\ \hline
\end{tabular}
\end{center}
...
\subsubsection{Attivit\`a svolte e dati trattati}
XMLParser contiene le operazioni per scorrere l'\underline{AST} ed effettuare la traduzione in XML. Dispone inoltre di una struttura dati statica per associare i valori numerici che il parser usa per identificare i \underline{token} ai nomi dei \underline{token}s definiti dall'utente.
\begin{center}
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
Attributo & Descrizione \\  \hline
\underline{-\underline{token}List:}\underline{ String[]}& Nell'\underline{AST} il tipo di \underline{token} viene rappresentato tramite un numero univoco. Questo numero corrisponde, per ogni \underline{token}, all'indice che in \underline{token}List contiene la rappresentazione testuale del \underline{token} stesso. Per poter associare in maniera rapida i \underline{token}s ai numeri che li rappresentano \`e dunque necessario appoggiarsi a questa struttura dati proveniente direttamente da \brp.\\ \hline
\end{tabular}
\end{center}
\begin{center}
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
Metodo & Descrizione \\  \hline
+XMLParser(tList: String[]) & Costruttore. In ingresso riceve la struttura dati che associa i \underline{token} ai numeri che li rappresentano.\\ \hline
+parse(\underline{AST}:Tree, rule:BusinessRule): String & Metodo pubblico che si incarica di effettuare la conversione da \underline{AST} a stringa che rappresenta l'elemento XML. Si dovr\`a seguire la procedura adeguata per la creazione di un elemento strutturato XML secondo le specifiche della libreria \textit{org.w3c.dom}\\ \hline

-scan\underline{AST}(\underline{AST}: Tree, root: Element, doc: Document) Element & Metodo privato che scorre ricorsivamente l'\underline{AST} per convertire la regola in un elemento XML. Questa verr\`a successivamente inserita nel \rp. \\ \hline
\end{tabular}
\end{center}
...
\subsubsection{Attivit\`a svolte e dati trattati}
La componente contiene i campi dato Stringa per rappresentare una \underline{business} \underline{rule} ossia name, associatedObject, rule e comment, quest'ultima pu\`o non essere istanziata. Vengono inoltre messi a disposizione il costruttore e il metodo ridefinito toString().
\begin{center}
\begin{tabular}{||p{6cm}||p{6cm}||} \hline
Attributo & Descrizione \\  \hline
