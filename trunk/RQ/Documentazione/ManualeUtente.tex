...
\subsection{Windows:}
L'installazionein sistemi Windows avviene attraverso un installer. Bisogna quindi accedere al cd rom di installazione e fare doppi clik sul file  \textbf{install.exe} il quale avvier\`a un wizard di installazione.
%inserire immagini e descrizioni 



\chapter{Avvio del sistema}
\subsection{Avvio di eXist}
Perch\`e il prodotto  sia avviato e funzioni correttamente \`e necessario che sia attivo il server eXist. Completata l'installazione quindi è sufficiente avviare eXist da uno dei collegamenti creati dall'installazione. Baster\`a un doppio click sull'icona ``eXist Database Startup'' creata.\\
...
\item Nei sistemi Windows fare doppio click sul collegamento brjsys.jar 
\end{itemize}
 nei sistemi linux
Selezionare il collegamento creato dall'installazione
