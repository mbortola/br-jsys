...
\end{center}
All'avvio della finestra di  \textit{inserimento business rule} questa richiede l'immissione dei relativi campi dati. Da qui si procede con l'estrazione del codice della regola per creare il relativo oggetto per la validazione. Viene quindi lanciato  \textit{org.antlr.runtime.BusinessRuleParser} che per prima cosa far\`a l' \textit{analisi lessicale} prelevando un carattere per volta fino alla terminazione della stringa o all'identificazione di un carattere inaspettato. Qualora venga riconosciuto un carattere inatteso verr\`a notificato l'errore e passato il controllo nuovamente alla finestra di inserimento. Se invece la lettura della stringa termina senza errori si passa all' \textit{analisi sintattica} ed al  \textit{controllo dei tipi} da parte del  \textit{parser}; tale operazione pu\`o: sollevare una  \textit{RecognitionException} in caso di errore sintattico, sollevare una  \textit{TypeCollisionException} in caso di errore nei tipi oppure incontrare un carattere inatteso. Nei primi due casi viene notificato l'errore e riportato il controllo alla finestra di inserimento, nel terzo caso invece si controlla se il carattere inatteso \`e uno spazio, ed in questo caso si torna all'analisi lessicale, oppure \`e un simbolo terminale. Se ci troviamo di fronte ad un carattere terminale possiamo istanziare l'oggetto  \textit{org.antlr.runtime.tree.Tree}. Questo si occuper\`a di estrarre l'AST e istanziare  \textit{org.antlr.runtime.tree.CommonTree} che creer\`a un documento XML. Il documento XML a questo punto istanzia l'oggetto  \textit{org.w3c.dom.Document} che a sua volta crea l'elemento radice business rule. A tale elemento vengono successvamente aggiunti gli attributi nome e business object associato e, se presente, l'attributo commento. A questo punto si pu\`o istanziare anche l'oggetto  \textit{org.w3c.dom.Element} per aggiungere l'elemento radice al documento XML. Fatto ci\`o \`e necessario creare una tabella hash per i nomi dei tag XML. Da qui partono una serie di operazioni in sequenza che si occupano di aprire il file, leggere una riga ed aggiungere il tag alla tabella hash fino a che il file non \`e terminato (in questo caso il file viene chiuso). A questo punto viene istanziato l'oggetto simbols che rappresenta la tabella hash. Si procede quindi all'analisi dell'AST che prevede: il recupero del nome del tag da simbols, la creazione dell'elemento XML corrispondente, l'aggiunta dei relativi attributi e quindi l'inserimento dell'elemento nel documento, fino alla terminazione della visita dell'albero. Il documento XML verr\`a quindi trasformato in stringa e i tag XML eliminati affinch\`e tale stringa possa essere inserita in un altro documento XML. Giunti a questo punto si verifica tramite una query l'univocit\`a del nome della business rule. Se l'esecuzione di questa query notifica che una business rule con lo stesso nome \`e gi\`a presente nel repository si torna alla finestra iniziale di inserimento dei campi dati. Nel caso in cui invece, il nome assegnato alla business rule sia disponibile viene  \textit{formulata ed eseguita una query di inserimento} della regola e quindi notificato l'avvenuto inserimento. A questo punto la finestra di inserimento pu\`o essere chiusa.
