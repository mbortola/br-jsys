...
\underline{DESCRIZIONE PROVA:}  ?\\modifica
\underline{INPUT:} ?\\
\underline{OUTPUT:} ?\\
\\
\underline{ID:} F2.   \\
\underline{OBIETTIVO PROVA:} Testare la correttezza sintattica di ogni Business Rules. \\
\underline{DESCRIZIONE PROVA:} Tentativo di inserimento/eliminazione di varie Business Rules d'appoggio (sintatticamente corrette o meno), per testarne il loro stato di accettazione.\\
\underline{INPUT:} Business Rules di appoggio.\\
\underline{OUTPUT:} Lo stato di accettazione delle Business Rules attraverso la notifica rilasciata.\\
\\
\underline{ID:} F3.\\
